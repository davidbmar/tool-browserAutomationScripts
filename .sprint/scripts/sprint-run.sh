...
merge_agents() {
  ...
  for agent in "${agents[@]}"; do
    ...
    # Clean ephemeral files before EACH merge — the pre-commit hook
    # rebuilds .index/ files on every commit, dirtying the working tree
    # and blocking the next merge with "local changes would be overwritten."
...
collect_metrics() {
  ...
  # TypeScript LOC
  local ts_loc
  ts_loc=$(find "${ROOT}/packages" "${ROOT}/services" -name '*.ts' -not -path '*/node_modules/*' -not -path '*/dist/*' | xargs wc -l 2>/dev/null | tail -1 | awk '{print $1}')
...
EOF
      ...
      for cmd in "${MERGE_VERIFY[@]}"; do
        ...
        if ! eval "$cmd" 2>&1; then
          ...
          echo "Fix issues, then re-run: $0 --continue"
          exit 1
        fi
      done
